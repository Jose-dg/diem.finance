#!/usr/bin/env python3
"""
Script para intercambiar créditos entre sellers.
Ejecutar con: python3 manage.py shell < scripts/swap_seller_credits.py
"""
...
def swap_seller_credits():
    ...
    try:
        ...
        with transaction.atomic():
            # Intercambiar sellers en los créditos
            Credit.objects.filter(seller=daniel_seller).update(seller=hector_seller)
            Credit.objects.filter(seller=hector_seller).update(seller=daniel_seller)
            
            # Pero espera, esto no funcionaría porque ya cambiamos todos a hector_seller
...
if __name__ == "__main__":
    print("🚀 Iniciando intercambio de créditos entre sellers...")
    
    # Mostrar estado inicial
    show_current_state()
    
    # Confirmar con el usuario
    response = input("¿Deseas proceder con el intercambio? (s/N): ")
